...
합성 음원 회귀 검증은 통과하지만, 첫 실제 회기 영상에서는 화자 판별이 거의 작동하지 않았고
전사도 비정상적으로 반복되었다. 현재는 이 괴리를 해결하는 단계의 프로토타입이며 임상 실사용
가능 상태가 아니다. 원인 진단은 `docs/tech/field_failure_diagnosis.md`에 기록되어 있다.
